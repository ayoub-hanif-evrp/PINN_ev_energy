"""Feature construction. LAT/LON/Date/Time/SoC are not main NN predictors."""
...
from data.schema import EXCLUDED_MAIN_PREDICTORS, MAIN_MODEL_FEATURES
...
def sample_feature_frame(trip: ProcessedTrip, feature_names: tuple[str, ...] | None = None) -> pd.DataFrame:
    ...
    forbidden = [n for n in names if n in EXCLUDED_MAIN_PREDICTORS]
    if forbidden:
...
def trip_level_features(trip: ProcessedTrip) -> dict[str, Any]:
    """Physically meaningful trip summaries for later ElasticNet (not used as NN inputs)."""
    df = trip.frame
    ...
    distance_m = float(df["s_m"].iloc[-1]) if len(df) else 0.0
    idle = speed < 0.5
    pos_acc = acc > 0.3
    neg_acc = acc < -0.3
